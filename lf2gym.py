...
def make(ip='127.0.0.1', port=8000, startServer=True, wrap='skip4', driverType=WebDriver.PhantomJS,
         characters=[Character.Davis,
                     Character.Dennis], difficulty=Difficulty.Dumbass,
         background=Background.HK_Coliseum, action_options=[
             'Basic', 'Advanced', 'Run', 'AJD', 'Full Combos'],
    ...
    if wrap is not None:
        if wrap == '4':
            ...
            env = LF2SkipNWrapper(
                env, num_frame, 4, characters[0], action_options, debug)
        else:
