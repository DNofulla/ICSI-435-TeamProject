    ...
    if driverType == 'PhantomJS':
        ...
        if localDriver:
            ...
            driver = webdriver.PhantomJS(executable_path=source, service_log_path=join(path, 'phantomjs.log'),
                                         service_args=["--remote-debugger-port=9000", "--web-security=false"])
            # driver = webdriver.PhantomJS(executable_path=source, service_args=["--remote-debugger-port=9000", "--web-security=false"])
        else:
            driver = webdriver.PhantomJS(service_log_path=join(path, 'phantomjs.log'),
                                         service_args=["--remote-debugger-port=9000", "--web-security=false"])
        ...
        if headless:
            # will not show the Chrome browser window
            chrome_options.add_argument("--headless")
        if localDriver:
            source = get_source(driverType, path)
            driver = webdriver.Chrome(executable_path=source, service_log_path=join(path, 'chromedriver.log'),
                                      desired_capabilities=desired, chrome_options=chrome_options)
        else:
            driver = webdriver.Chrome(service_log_path=join(path, 'chromedriver.log'), desired_capabilities=desired,
        ...
        if localDriver:
            source = get_source(driverType, path)
            driver = webdriver.Firefox(executable_path=source, service_log_path=join(path, 'geckodriver.log'),
                                       firefox_options=firefox_options)
        else:
            driver = webdriver.Firefox(service_log_path=join(
                path, 'geckodriver.log'), firefox_options=firefox_options)
    return driver
...
def get_source(driverType, path='.'):
    ...
    if driverType == 'PhantomJS':
        if os == 'Windows':
            source = join(
                path, 'webdriver/phantomjsdriver_2.1.1_win32/phantomjs.exe')
        elif os == 'Darwin':
            source = join(
                path, 'webdriver/phantomjsdriver_2.1.1_mac64/phantomjs')
        elif os == 'Linux' and bits == '32bit':
            source = join(
                path, 'webdriver/phantomjsdriver_2.1.1_linux32/phantomjs')
        elif os == 'Linux' and bits == '64bit':
            source = join(
                path, 'webdriver/phantomjsdriver_2.1.1_linux64/phantomjs')
        else:
            raise Exception(
                'Failed to recognize your OS [%s / %s].' % (os, bits))
    elif driverType == 'Chrome':
        if os == 'Windows':
            source = join(
                path, 'webdriver/chromedriver_2.39_win32/chromedriver.exe')
        elif os == 'Darwin':
            source = join(
                path, 'webdriver/chromedriver_2.39_mac64/chromedriver')
        elif os == 'Linux':
            source = join(
                path, 'webdriver/chromedriver_2.39_linux64/chromedriver')
        else:
            raise Exception(
                'Failed to recognize your OS [%s / %s].' % (os, bits))
    elif driverType == 'Firefox':
        if os == 'Windows' and bits == '32bit':
            source = join(
                path, 'webdriver/geckodriver_0.19.1_win32/geckodriver.exe')
        elif os == 'Windows' and bits == '64bit':
            source = join(
                path, 'webdriver/geckodriver_0.19.1_win64/geckodriver.exe')
        elif os == 'Darwin':
            source = join(
                path, 'webdriver/geckodriver_0.19.1_macos/geckodriver')
        elif os == 'Linux' and bits == '32bit':
            source = join(
                path, 'webdriver/geckodriver_0.19.1_linux32/geckodriver')
        elif os == 'Linux' and bits == '64bit':
            source = join(
                path, 'webdriver/geckodriver_0.19.1_linux64/geckodriver')
        else:
            raise Exception(
                'Failed to recognize your OS [%s / %s].' % (os, bits))
    else:
